...
["PIXI", "../../lib/pixi.dart", , M, {
  ...
  hex2rgb: function(hex) {
    if (typeof hex !== "number")
      return hex.$shr();
    return H.assertSubtype([(hex >>> 16 & 255) / 255, (hex >>> 8 & 255) / 255, (hex & 255) / 255], "$isList", [P.num], "$asList");
  },
  Matrix: {
  ...
  Rope: {
    "^": "Strip;points,count:PIXI$Rope$count<,texture,blendMode,uvs,verticies,colors,indices,updateFrame,_vertexBuffer,_indexBuffer,_uvBuffer,_colorBuffer,count,children,interactiveChildren,_width,_height,position,scale,pivot,rotation,alpha,visible,hitArea,buttonMode,renderable,_parent,__hit,__isOver,__mouseIsDown,__isDown,_dirty,click,mousemove,mousedown,mouseout,mouseover,mouseup,mouseupoutside,touchmove,touchstart,touchend,tap,touchendoutside,__touchData,_stage,_worldAlpha,_interactive,defaultCursor,_worldTransform,_sr,_cr,filterArea,_bounds,_currentBounds,_mask,_cacheAsBitmap,_cachedSprite,_cacheIsDirty,_filterBlock,_filters,_rotationCache",
    set$count: function(count) {
      this.PIXI$Rope$count = H.numTypeCheck(count);
    },
    refresh$0: function() {
      var t1, uvs, indices, colors, t2, t3, t4, t5, i, index, amount, t6, t7, t8;
      t1 = H.assertSubtype(this.points, "$isList", [M.Point], "$asList").length;
      if (t1 < 1)
        return;
      uvs = this.uvs;
      indices = this.indices;
      colors = this.colors;
      this.PIXI$Rope$count -= 0.2;
      t2 = uvs.length;
      if (0 >= t2)
        return H.ioore(uvs, 0);
      uvs[0] = 0;
      if (1 >= t2)
        return H.ioore(uvs, 1);
      uvs[1] = 0;
      if (2 >= t2)
        return H.ioore(uvs, 2);
      uvs[2] = 0;
      if (3 >= t2)
        return H.ioore(uvs, 3);
      uvs[3] = 1;
      t3 = colors.length;
      if (0 >= t3)
        return H.ioore(colors, 0);
      colors[0] = 1;
      if (1 >= t3)
        return H.ioore(colors, 1);
      colors[1] = 1;
      t4 = indices.length;
      if (0 >= t4)
        return H.ioore(indices, 0);
      indices[0] = 0;
      if (1 >= t4)
        return H.ioore(indices, 1);
      indices[1] = 1;
      for (t5 = t1 - 1, i = 1; i < t1; ++i) {
        index = i * 4;
        amount = i / t5;
        t6 = index + 2;
        t7 = index + 1;
        ...
        if (C.JSInt_methods.$mod(i, 2) !== 0) {
          if (index >= t2)
            return H.ioore(uvs, index);
          uvs[index] = amount;
          if (t7 >= t2)
            return H.ioore(uvs, t7);
          uvs[t7] = 0;
          if (t6 >= t2)
            return H.ioore(uvs, t6);
          uvs[t6] = amount;
          if (t8 >= t2)
            return H.ioore(uvs, t8);
          uvs[t8] = 1;
        } else {
          if (index >= t2)
            return H.ioore(uvs, index);
        ...
        t6 = index + 1;
        if (t6 >= t3)
          return H.ioore(colors, t6);
        colors[t6] = 1;
        if (index >= t4)
          return H.ioore(indices, index);
        indices[index] = index;
        if (t6 >= t4)
          return H.ioore(indices, t6);
        indices[t6] = t6;
      }
    },
    _updateTransform$0: function() {
      var points, t1, lastPoint, perp, verticies, t2, t3, i, t4, point, index, nextPoint, t5, perpLength, n, t6;
      points = H.assertSubtype(this.points, "$isList", [M.Point], "$asList");
      t1 = points.length;
      if (t1 < 1)
        return;
      lastPoint = points[0];
      perp = new M.Point(null, null);
      perp.x = 0;
      perp.y = 0;
      this.PIXI$Rope$count -= 0.2;
      verticies = this.verticies;
      for (t2 = verticies.length, t3 = t1 - 1, i = 0; i < t1; ++i, lastPoint = point) {
        t4 = points.length;
        if (i >= t4)
          return H.ioore(points, i);
        point = points[i];
        index = i * 4;
        nextPoint = i < t4 - 1 ? points[i + 1] : point;
        t4 = nextPoint.x;
        t5 = lastPoint.x;
        if (typeof t4 !== "number")
          return t4.$sub();
        perp.y = -C.JSNumber_methods.$sub(t4, t5);
        t5 = nextPoint.y;
        t4 = lastPoint.y;
        if (typeof t5 !== "number")
          return t5.$sub();
        t4 = C.JSNumber_methods.$sub(t5, t4);
        perp.x = t4;
        if ((1 - i / t3) * 10 > 1)
          ;
        t5 = perp.y;
        if (typeof t5 !== "number")
          return t5.$mul();
        perpLength = Math.sqrt(t4 * t4 + t5 * t5);
        t4 = this.texture.PIXI$Texture$height;
        if (typeof t4 !== "number")
          return t4.$div();
        n = t4 / 2;
        t4 = perp.x;
        ...
        verticies[index] = t5 + t4;
        t4 = index + 1;
        t5 = point.y;
        t6 = J.toDouble$0$n(perp.y);
        if (typeof t5 !== "number")
          return t5.$add();
        if (t4 >= t2)
          return H.ioore(verticies, t4);
        verticies[t4] = t5 + t6;
        t6 = index + 2;
        t5 = point.x;
        t4 = J.toDouble$0$n(perp.x);
        if (typeof t5 !== "number")
          return t5.$sub();
        if (t6 >= t2)
          return H.ioore(verticies, t6);
        verticies[t6] = t5 - t4;
        t4 = index + 3;
        t5 = point.y;
        t6 = J.toDouble$0$n(perp.y);
    ...
    Rope$2: function(texture, points) {
      var t1;
      H.assertSubtype(points, "$isList", [M.Point], "$asList");
      t1 = this.points.length;
      this.verticies = new Float32Array(t1 * 4);
      t1 = this.points.length;
      this.uvs = new Float32Array(t1 * 4);
      t1 = this.points.length;
      this.colors = new Float32Array(t1 * 2);
      t1 = this.points.length;
      this.indices = new Uint16Array(t1 * 2);
      this.refresh$0();
    },
    $isRope: true
  },
  Strip: {
    ...
    set$count: function(count) {
      this.count = H.intTypeCheck(count);
    },
    _renderWebGL$1: function(renderSession) {
      var gl, t1, projection, offset, shader, t2, array, t3;
      if (!this.visible || this.alpha <= 0)
        return;
      ...
      if (this._vertexBuffer == null) {
        ...
        C.RenderingContext_methods.bindBuffer$2(gl, 34962, this._vertexBuffer);
        C.RenderingContext_methods.bufferData$3(gl, 34962, this.verticies, 35048);
        C.RenderingContext_methods.bindBuffer$2(gl, 34962, this._uvBuffer);
        C.RenderingContext_methods.bufferData$3(gl, 34962, this.uvs, 35044);
        C.RenderingContext_methods.bindBuffer$2(gl, 34962, this._colorBuffer);
        C.RenderingContext_methods.bufferData$3(gl, 34962, this.colors, 35044);
        C.RenderingContext_methods.bindBuffer$2(gl, 34963, this._indexBuffer);
        C.RenderingContext_methods.bufferData$3(gl, 34963, this.indices, 35044);
      }
      t1 = renderSession.shaderManager;
      t1.setShader$1(t1.stripShader);
      gl = renderSession.gl;
      projection = renderSession.projection;
      offset = renderSession.offset;
      shader = renderSession.shaderManager.stripShader;
      J.getInterceptor$x(gl).blendFunc$2(gl, 1, 771);
      t1 = shader.PIXI$StripShader$translationMatrix;
      ...
      array[7] = t2.ty;
      array[8] = 1;
      C.RenderingContext_methods.uniformMatrix3fv$3(gl, t1, false, array);
      t1 = shader.PIXI$StripShader$projectionVector;
      t2 = projection.x;
      t3 = projection.y;
      if (typeof t3 !== "number")
        return t3.$negate();
      C.RenderingContext_methods.uniform2f$3(gl, t1, t2, -t3);
      t3 = shader.PIXI$StripShader$offsetVector;
      t2 = offset.x;
      if (typeof t2 !== "number")
        return t2.$negate();
      t1 = offset.y;
      if (typeof t1 !== "number")
        return t1.$negate();
      C.RenderingContext_methods.uniform2f$3(gl, t3, -t2, -t1);
      C.RenderingContext_methods.uniform1f$2(gl, shader.PIXI$StripShader$alpha, 1);
      if (!this._dirty) {
        C.RenderingContext_methods.bindBuffer$2(gl, 34962, this._vertexBuffer);
        C.RenderingContext_methods.bufferSubData$3(gl, 34962, 0, this.verticies);
        C.RenderingContext_methods.vertexAttribPointer$6(gl, shader.PIXI$StripShader$aVertexPosition, 2, 5126, false, 0, 0);
        C.RenderingContext_methods.bindBuffer$2(gl, 34962, this._uvBuffer);
        C.RenderingContext_methods.vertexAttribPointer$6(gl, shader.PIXI$StripShader$aTextureCoord, 2, 5126, false, 0, 0);
        C.RenderingContext_methods.activeTexture$1(gl, 33984);
        t1 = this.texture.baseTexture._glTextures.$index(0, gl);
        t2 = this.texture;
        C.RenderingContext_methods.bindTexture$2(gl, 3553, t1 == null ? M.createWebGLTexture(t2.baseTexture, gl) : t2.baseTexture._glTextures.$index(0, gl));
        C.RenderingContext_methods.bindBuffer$2(gl, 34963, this._indexBuffer);
      } else {
        this._dirty = false;
        C.RenderingContext_methods.bindBuffer$2(gl, 34962, this._vertexBuffer);
        C.RenderingContext_methods.bufferData$3(gl, 34962, this.verticies, 35044);
        C.RenderingContext_methods.vertexAttribPointer$6(gl, shader.PIXI$StripShader$aVertexPosition, 2, 5126, false, 0, 0);
        C.RenderingContext_methods.bindBuffer$2(gl, 34962, this._uvBuffer);
        C.RenderingContext_methods.bufferData$3(gl, 34962, this.uvs, 35044);
        C.RenderingContext_methods.vertexAttribPointer$6(gl, shader.PIXI$StripShader$aTextureCoord, 2, 5126, false, 0, 0);
      ...
      C.RenderingContext_methods.drawElements$4(gl, 5, this.indices.length, 5123, 0);
      renderSession.spriteBatch.dirty = true;
    },
    _renderCanvas$1: function(renderSession) {
      var context, transform, t1, verticies, uvs, t2, t3, i, index, x0, t4, x1, t5, x2, t6, y0, t7, y1, t8, y2, centerX, centerY, normX, normY, dist, t9, u0, u1, u2, v0, v1, v2, delta, t10, t11, t12, t13;
      ...
      if (H.boolConversionCheck(renderSession.roundPixels))
        t1.setTransform$6(context, transform.a, transform.c, transform.b, transform.d, C.JSDouble_methods.toInt$0(H.doubleTypeCheck(Math.floor(transform.tx))), C.JSDouble_methods.toInt$0(H.doubleTypeCheck(Math.floor(transform.ty))));
      else
      ...
      for (t2 = t1 / 2 - 2, t3 = uvs.length, i = 0; i < t2; ++i) {
        index = i * 2;
        if (index >= t1)
          return H.ioore(verticies, index);
        x0 = verticies[index];
        ...
        t7 = index + 3;
        if (t7 >= t1)
          return H.ioore(verticies, t7);
        y1 = verticies[t7];
        t8 = index + 5;
        if (t8 >= t1)
          return H.ioore(verticies, t8);
        y2 = verticies[t8];
        centerX = (x0 + x1 + x2) / 3;
        centerY = (y0 + y1 + y2) / 3;
        normX = x0 - centerX;
        ...
        t9 = dist + 3;
        x0 = centerX + normX / dist * t9;
        y0 = centerY + normY / dist * t9;
        normX = x1 - centerX;
        normY = y1 - centerY;
        dist = Math.sqrt(normX * normX + normY * normY);
        t9 = dist + 3;
        x1 = centerX + normX / dist * t9;
        y1 = centerY + normY / dist * t9;
        normX = x2 - centerX;
        normY = y2 - centerY;
        dist = Math.sqrt(normX * normX + normY * normY);
        t9 = dist + 3;
        x2 = centerX + normX / dist * t9;
        y2 = centerY + normY / dist * t9;
        if (index >= t3)
          return H.ioore(uvs, index);
        u0 = C.JSNumber_methods.$mul(uvs[index], this.texture.PIXI$Texture$width);
        if (t4 >= t3)
          return H.ioore(uvs, t4);
        u1 = C.JSNumber_methods.$mul(uvs[t4], this.texture.PIXI$Texture$width);
        if (t5 >= t3)
          return H.ioore(uvs, t5);
        u2 = C.JSNumber_methods.$mul(uvs[t5], this.texture.PIXI$Texture$width);
        if (t6 >= t3)
          return H.ioore(uvs, t6);
        v0 = C.JSNumber_methods.$mul(uvs[t6], this.texture.PIXI$Texture$height);
        if (t7 >= t3)
          return H.ioore(uvs, t7);
        v1 = C.JSNumber_methods.$mul(uvs[t7], this.texture.PIXI$Texture$height);
        if (t8 >= t3)
          return H.ioore(uvs, t8);
        v2 = C.JSNumber_methods.$mul(uvs[t8], this.texture.PIXI$Texture$height);
        context.save();
        context.beginPath();
        C.CanvasRenderingContext2D_methods.moveTo$2(context, x0, y0);
        C.CanvasRenderingContext2D_methods.lineTo$2(context, x1, y1);
        C.CanvasRenderingContext2D_methods.lineTo$2(context, x2, y2);
        context.closePath();
        context.clip();
        t4 = u0 * v1;
        t5 = v0 * u1;
        delta = t4 + v0 * u2 + u1 * v2 - v1 * u2 - t5 - u0 * v2;
        t6 = x0 * v1;
        t7 = v0 * x1;
        t8 = u0 * x1;
        t9 = x0 * u1;
        ...
        t12 = u0 * y1;
        t13 = y0 * u1;
        C.CanvasRenderingContext2D_methods.transform$6(context, (t6 + v0 * x2 + x1 * v2 - v1 * x2 - t7 - x0 * v2) / delta, (t10 + v0 * y2 + y1 * v2 - v1 * y2 - t11 - y0 * v2) / delta, (t8 + x0 * u2 + u1 * x2 - x1 * u2 - t9 - u0 * x2) / delta, (t12 + y0 * u2 + u1 * y2 - y1 * u2 - t13 - u0 * y2) / delta, (t4 * x2 + t7 * u2 + t9 * v2 - t6 * u2 - t5 * x2 - t8 * v2) / delta, (t4 * y2 + t11 * u2 + t13 * v2 - t10 * u2 - t5 * y2 - t12 * v2) / delta);
        C.CanvasRenderingContext2D_methods.drawImage$3(context, this.texture.baseTexture.source, 0, 0);
        context.restore();
      }
    },
    Strip$1: function(texture) {
      ...
      H.assertSubtype(t1, "$isList", [P.$double], "$asList");
      this.uvs = new Float32Array(H._ensureNativeList(t1));
      t1 = [0, 0, 100, 0, 100, 100, 0, 100];
      H.assertSubtype(t1, "$isList", [P.$double], "$asList");
      H.assertSubtype(t1, "$isList", [P.$double], "$asList");
      this.verticies = new Float32Array(H._ensureNativeList(t1));
      t1 = [1, 1, 1, 1];
      H.assertSubtype(t1, "$isList", [P.$double], "$asList");
      H.assertSubtype(t1, "$isList", [P.$double], "$asList");
      ...
      H.assertSubtype(t1, "$isList", [P.$int], "$asList");
      this.indices = new Uint16Array(H._ensureNativeList(t1));
      this._dirty = true;
    }
  },
  AbstractFilter: {
  ...
  InteractionManager: {
    ...
    }, "call$1", "get$onMouseMove", 2, 0, 53, 2],
    ...
    }, "call$1", "get$onMouseDown", 2, 0, 54, 2],
    onMouseOut$1: [function(_, $event) {
      ...
      t1.x = -10000;
      t1.y = -10000;
    }, "call$1", "get$onMouseOut", 2, 0, 54, 2],
    ...
    hitTest$2: function(item, interactionData) {
      ...
      t4 = C.JSNumber_methods.$mul(a00 * id, global.y);
      t1 = C.JSNumber_methods.$mul(t1 * id, global.x);
      ...
      if (item.get$hitArea() != null) {
        ...
        t5 = false;
      if (t5) {
        if (item.get$hitArea().contains$2(0, t2 + t3 + (a12 * a01 - a02 * a11) * id, t4 + t1 + (-a12 * a00 + a02 * a10) * id)) {
          interactionData.target = item;
          return true;
        }
        return false;
    ...
    }, "call$1", "get$onTouchMove", 2, 0, 14, 2],
    ...
    }, "call$1", "get$onTouchStart", 2, 0, 14, 2],
    ...
    }, "call$1", "get$onTouchEnd", 2, 0, 14, 2],
  ...
  StripShader: {
    "^": "Shader;program<,gl,fragmentSrc,vertexSrc,PIXI$StripShader$uSampler,PIXI$StripShader$projectionVector,PIXI$StripShader$offsetVector,PIXI$StripShader$dimensions,PIXI$StripShader$uMatrix,PIXI$StripShader$translationMatrix,PIXI$StripShader$alpha,PIXI$StripShader$aVertexPosition,PIXI$StripShader$aPositionCoord,PIXI$StripShader$aScale,PIXI$StripShader$aRotation,PIXI$StripShader$aTextureCoord,PIXI$StripShader$colorAttribute,attributes:PIXI$StripShader$attributes>,uSampler,projectionVector,offsetVector,dimensions,uMatrix,tintColor,color,translationMatrix,alpha,aVertexPosition,aPositionCoord,aScale,aRotation,aTextureCoord,colorAttribute,attributes,_UID",
    init$0: function() {
      var program, t1;
      program = M.compileProgram(this.gl, this.vertexSrc, this.fragmentSrc);
      J.useProgram$1$x(this.gl, program);
  ...
  WebGLRenderer: {
    ...
    }, "call$1", "get$handleContextLost", 2, 0, 30, 2],
    ...
    }, "call$1", "get$handleContextRestored", 2, 0, 30, 2],
  ...
  Texture: {
    ...
    }, "call$0", "get$onBaseTextureLoaded", 0, 0, 13],
    ...
    Texture$2: function(baseTexture, frame) {
      ...
      if (baseTexture._hasLoaded) {
        ...
        if (H.Lists_indexOf(t3, t1, 0, t3.length) === -1)
          J.insert$2$ax(t2.$index(0, "loaded"), 0, t1);
      }
    },
    $isTexture: true,
...
["_interceptors", "dart:_interceptors", , J, {
  ...
  Interceptor: {
    ...
    "%": "ArrayBuffer|CanvasGradient|CanvasPattern|DOMError|FileError|MediaKeyError|Navigator|NavigatorUserMediaError|PositionError|SQLError|SVGAnimatedEnumeration|SVGAnimatedLengthList|SVGAnimatedNumber|SVGAnimatedNumberList|SVGAnimatedTransformList|SVGPointList|SVGRect|WebGLRenderbuffer"
  },
  JSBool: {
    "^": "Interceptor;",
    toString$0: function(receiver) {
...
["_isolate_helper", "dart:_isolate_helper", , H, {
  ...
  IsolateNatives__startNonWorker_closure: {
    "^": "Closure:33;box_0,functionName_1,isSpawnUri_2,startPaused_3,replyPort_4",
    call$0: function() {
      var t1 = this.box_0;
...
["_js_helper", "dart:_js_helper", , H, {
  ...
  Primitives_functionNoSuchMethod_closure: {
    "^": "Closure:60;box_0,arguments_1,namedArgumentList_2",
  ...
  Primitives_applyFunction_closure: {
    "^": "Closure:60;box_0,defaultArguments_1",
...
["dart.async", "dart:async", , P, {
  ...
  _Future__propagateToListeners_handleValueCallback: {
    "^": "Closure:63;box_1,listener_3,sourceValue_4,zone_5",
    call$0: function() {
  ...
  _ForwardingStreamSubscription: {
    ...
    }, "call$2", "get$_handleError", 4, 0, 69, 18, 19],
  ...
  _rootHandleUncaughtError__closure: {
    "^": "Closure:33;error_2,stackTrace_3",
    call$0: function() {
      var t1, trace;
      t1 = this.error_2;
...
["dart.dom.html", "dart:html", , W, {
  ...
  CanvasRenderingContext2D: {
    ...
    drawImage$3: function(receiver, source, destX, destY) {
      return receiver.drawImage(H.interceptedTypeCheck(source, "$isCanvasImageSource"), destX, destY);
    },
...
["dart.dom.web_gl", "dart:web_gl", , P, {
  ...
  RenderingContext: {
    ...
    }, "call$3", "get$uniformMatrix3fv", 6, 0, null, 36, 50, 51],
...
["", "main.dart", , F, {
  "^": "",
  main: [function() {
    var t1, t2, t3, t4, t5, t6, t7, t8, t9, stage, renderer, target, points, i, texture, baseTexture, crossorigin, image, strip, snakeContainer;
    t1 = {};
    t1.count_0 = 0;
    ...
    texture = H.interceptedTypeCheck($.get$TextureCache().$index(0, "snake.png"), "$isTexture");
    if (texture == null) {
      baseTexture = $.get$BaseTextureCache().$index(0, "snake.png");
      crossorigin = C.JSString_methods.indexOf$1("snake.png", "data:") === -1 ? true : null;
      if (baseTexture == null) {
        ...
        baseTexture.imageUrl = "snake.png";
        $.get$BaseTextureCache().$indexSet(0, "snake.png", baseTexture);
      }
      texture = M.Texture$(baseTexture, null);
      $.get$TextureCache().$indexSet(0, "snake.png", texture);
    }
    H.assertSubtype(points, "$isList", [M.Point], "$asList");
    t2 = new M.Point(null, null);
    t2.x = 0;
    ...
    strip = new M.Rope(H.assertSubtype(points, "$isList", [M.Point], "$asList"), 0, null, C.BlendModes_0, null, null, null, null, false, null, null, null, null, 0, H.assertSubtype([], "$isList", [M.DisplayInterface], "$asList"), false, null, null, t2, t3, t4, 0, 1, true, null, false, false, null, false, false, false, false, false, t9._assertCheck$1(null), t9._assertCheck$1(null), t9._assertCheck$1(null), t9._assertCheck$1(null), t9._assertCheck$1(null), t9._assertCheck$1(null), t9._assertCheck$1(null), t9._assertCheck$1(null), t9._assertCheck$1(null), t9._assertCheck$1(null), t9._assertCheck$1(null), t9._assertCheck$1(null), H.assertSubtype(t5, "$isMap", [P.$int, M.InteractionData], "$asMap"), null, 1, false, "pointer", new M.Matrix(1, 0, 0, 1, 0, 0, t6), 0, 1, null, new M.Rectangle(0, 0, 1, 1), null, null, false, null, false, new M.FilterBlock(true, true, null, null, null, t7, null), H.assertSubtype(null, "$isList", [M.AbstractFilter], "$asList"), 0);
    strip.Strip$1(texture);
    strip.Rope$2(texture, points);
    t2.x = -459;
    t2 = new M.Point(null, null);
    t2.x = 0;
    t2.y = 0;
    ...
    snakeContainer = new M.DisplayObjectContainer(t8, false, null, null, t2, t7, t6, 0, 1, true, null, false, false, null, false, false, false, false, false, t9._assertCheck$1(null), t9._assertCheck$1(null), t9._assertCheck$1(null), t9._assertCheck$1(null), t9._assertCheck$1(null), t9._assertCheck$1(null), t9._assertCheck$1(null), t9._assertCheck$1(null), t9._assertCheck$1(null), t9._assertCheck$1(null), t9._assertCheck$1(null), t9._assertCheck$1(null), H.assertSubtype(t5, "$isMap", [P.$int, M.InteractionData], "$asMap"), null, 1, false, "pointer", new M.Matrix(1, 0, 0, 1, 0, 0, t3), 0, 1, null, new M.Rectangle(0, 0, 1, 1), null, null, false, null, false, new M.FilterBlock(true, true, null, null, null, t4, null), H.assertSubtype(null, "$isList", [M.AbstractFilter], "$asList"), 0);
    ...
    t7.x = t4;
    t7.y = t4;
    stage.addChildAt$2(snakeContainer, stage.children.length);
    snakeContainer.addChildAt$2(strip, t8.length);
    $.get$requestAnimFrame().call$1(new F.main_animate(t1, stage, renderer, points));
  }, "call$0", "main$closure", 0, 0, 33],
  main_animate: {
    "^": "Closure:30;box_0,stage_1,renderer_2,points_3",
    call$1: [function(dt) {
      var t1, t2, i, t3, t4;
      t1 = this.box_0;
      t1.count_0 += 0.1;
      for (t2 = this.points_3, i = 0; i < t2.length; ++i) {
        t3 = t2[i];
        t4 = t1.count_0;
        t3.y = Math.sin(i * 0.5 + t4) * 30;
        if (i >= t2.length)
          return H.ioore(t2, i);
        t3 = t2[i];
        t4 = t1.count_0;
        t3.x = i * 45.9 + Math.cos(i * 0.3 + t4) * 20;
      }
...
function dart_precompiled($collectedClasses) {
  ...
  function Rope(points, PIXI$Rope$count, texture, blendMode, uvs, verticies, colors, indices, updateFrame, _vertexBuffer, _indexBuffer, _uvBuffer, _colorBuffer, count, children, interactiveChildren, _width, _height, position, scale, pivot, rotation, alpha, visible, hitArea, buttonMode, renderable, _parent, __hit, __isOver, __mouseIsDown, __isDown, _dirty, click, mousemove, mousedown, mouseout, mouseover, mouseup, mouseupoutside, touchmove, touchstart, touchend, tap, touchendoutside, __touchData, _stage, _worldAlpha, _interactive, defaultCursor, _worldTransform, _sr, _cr, filterArea, _bounds, _currentBounds, _mask, _cacheAsBitmap, _cachedSprite, _cacheIsDirty, _filterBlock, _filters, _rotationCache) {
    this.points = points;
    this.PIXI$Rope$count = PIXI$Rope$count;
    this.texture = texture;
    ...
    this._uvBuffer = _uvBuffer;
    this._colorBuffer = _colorBuffer;
    this.count = count;
    this.children = children;
    this.interactiveChildren = interactiveChildren;
    this._width = _width;
  ...
  Rope.builtin$cls = "Rope";
  if (!"name" in Rope)
    Rope.name = "Rope";
  $desc = $collectedClasses.Rope;
  if ($desc instanceof Array)
    $desc = $desc[1];
  Rope.prototype = $desc;
  Rope.prototype.get$count = function() {
    return this.PIXI$Rope$count;
  };
  function Strip(count) {
    this.count = count;
  }
  Strip.builtin$cls = "Strip";
  if (!"name" in Strip)
    Strip.name = "Strip";
  $desc = $collectedClasses.Strip;
  if ($desc instanceof Array)
    $desc = $desc[1];
  Strip.prototype = $desc;
  Strip.prototype.get$count = function() {
    return this.count;
  };
  ...
  StripShader.prototype.get$program = function() {
    return this.program;
  };
  StripShader.prototype.get$attributes = function(receiver) {
    return this.PIXI$StripShader$attributes;
  };
  ...
  function main_animate(box_0, stage_1, renderer_2, points_3) {
    this.box_0 = box_0;
    this.stage_1 = stage_1;
    this.renderer_2 = renderer_2;
    this.points_3 = points_3;
  }
